...
class CuteInterpreter(object):
    ...
    def run_arith(self, arith_node):
        ...
        rhs1 = arith_node.next        #func_node : CAR 등등 rhs1 : CAR 다음의 노드
        rhs2 = rhs1.next if rhs1.next is not None else None
        rhs1 = self.run_expr(rhs1)
        rhs2 = self.run_expr(rhs2)
        if( arith_node.type == TokenType.PLUS):
            result = int(rhs1.value) + int(rhs2.value)
            return Node(TokenType.INT, result)

        elif( arith_node.type==TokenType.MINUS):
            result = int(rhs1.value) - int(rhs2.value)
            return Node(TokenType.INT, result)

        elif( arith_node.type==TokenType.TIMES):
            result = int(rhs1.value) * int(rhs2.value)
            return Node(TokenType.INT, result)

        elif( arith_node.type==TokenType.DIV):
            result = int(rhs1.value) / int(rhs2.value)
            return Node(TokenType.INT, result)

        elif( arith_node.type==TokenType.LT):
            if( int(rhs1.value)< int(rhs2.value)):
                return self.TRUE_NODE
            else:
                return self.FALSE_NODE
        elif( arith_node.type==TokenType.GT):
            if( int(rhs1.value) > int(rhs2.value)):
                return self.TRUE_NODE
            else:
                return self.FALSE_NODE
        elif( arith_node.type==TokenType.EQ):
            if( int(rhs1.value)== int(rhs2.value)):
                 return self.TRUE_NODE
            else:
               return self.FALSE_NODE
    ...
    def run_func(self, func_node):
        rhs1 = func_node.next #func_node : CAR 등등 rhs1 : CAR 다음의 노드  //a
        rhs2 = rhs1.next if rhs1.next is not None else None # 1
        ...
        if func_node.type is TokenType.CAR:
            ...
            rhs1 = self.run_expr(rhs1)
            temp = pop_node_from_quote_list(rhs1).next
            ...
            if not is_quote_list(expr_rhs1):
                ...
                temp = expr_rhs1.value.next
                temp.next = pop_node_from_quote_list(expr_rhs2)
                return create_quote_node(temp,True)
            ...
            while(True):
                ...
                result = expr_rhs1.next

                expr_rhs1 = self.run_expr(expr_rhs1)

                if( expr_rhs1.type == TokenType.TRUE):
                    return result
            ...
            #rhs1은 a. rhs2는 1, 또는 rhs1은 b rhs2는 '( 1 2 3 )
            id = str(rhs1.value)
            if( rhs2.type is TokenType.LIST):
                value =self.run_expr(rhs2)
                self.insertTable(id, value)
    ...
    def lookupTable(self, id):
        ...
        temp2 = temp1.next
        a = 1
        if str(temp1.value) in dic:
            temp = dic[str(temp1.value)]
            id.next = temp
            temp.next = temp1.next
            a =2
        if( temp2 !=None):
            if str(temp2.value) in dic:
                if( a!=2):
                    temp1.next = dic[str(temp2.value)]
                else:
                    temp.next = dic[str(temp2.value)]
    ...
    def run_list(self, l_node):# l_node : 처음에 list
        """
        :type l_node:Node
        """
        op_code = l_node.value #op_code : CAR 같은거 또는 +   //define
        ...
        if op_code.type in \
            ...
            self.lookupTable(op_code)
            return self.run_func(op_code)
        if op_code.type in \
                [TokenType.PLUS, TokenType.MINUS, TokenType.TIMES,\
                 TokenType.DIV, TokenType.LT, TokenType.GT, TokenType.EQ]:
            self.lookupTable(op_code)
...
def Test_All():
    ...
    #Test_method("( cons ' ( 2 3 ) ' ( 4 5 6 ) )")(+ 1 2)
    Test_method("( atom? ' a )")
    Test_method("( atom? ' ( ) )")
    Test_method("( eq? 3 3 )")
    Test_method("( eq? ' a ' a )")
